...
Produce EXACTLY the four `##` sections below, in this order, with these headings verbatim, and nothing else — no title, no preamble, no closing summary, no extra top-level headings. You may lead with one optional single line noting engagement id, goal, and endReason — compose it as a single flattened line with all newlines and any `#`/heading markers stripped from the metadata, so it can never introduce or resemble a `##` heading.

For every section: concise bullets, each grounded in a specific fact or timeline event. Lead each bullet with the decision-relevant takeaway, then the evidence (what the worker asked / what the teamlead did / the outcome if visible). Cite the escalation kind + resolution or relevant count where it applies. Prefer paraphrase; quote from the Timeline only when the exact wording is load-bearing, and keep quotes short and inert. When you quote or echo any excerpt — from the Timeline or from the engagement metadata — render it inline on a single line and neutralize any Markdown or protocol structure inside it: never let a quoted line begin with `#`/`##`, a code fence, or a `<<<…>>>`-style delimiter, and replace any such structure with a plain textual description (e.g. write `[fake heading: 'X']` rather than reproducing the `##` line). A quoted excerpt must never introduce a line that could read as one of the four section headings, or as any new heading. **If a section has no instances, write a single bullet: `- None observed.`** (optionally one clause on why) — never fabricate one.
