...
| Managed-block harmonization pass | Templates | S | done | Opus 5 re-read of the in-use blocks: testing v20 ("real surface" pinned to the tier that fits the change per `docs/tests.md`, explicitly not necessarily E2E — the prior wording pulled agents toward E2E on trivial changes, contradicting the tests.md tier table), conventions v45 (the `'Checkpoint' = commit and continue` rule now states that it overrides the harness default to commit only when asked). Open, not done: the conventions worktree bullet has no escape hatch for a launcher that configured the session to work in place (its `git rev-parse` test only detects "already inside a worktree"); `active-work` restates "don't accumulate history" four ways but compressing it saves only ~14% (~110 tokens/session), so it was left alone. | 2026-07-26 |
