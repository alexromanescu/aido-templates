...
- **Fresh-eyes review before merge** for anything bigger than an S-sized quick fix: an independent review of the diff (`residuals-review` skill where available) — no shared author context, every finding acted on adversarially verified, verifier defaulting to REFUTED — run before the merge, while the author's context is loaded. When a cycle produced fixes, a fresh cycle reviews the post-fix diff — the merge gate is one clean pass over what actually merges. In-area findings are fixed in the same cycle; out-of-area ones filed with the reason named. If the harness cannot provide genuinely independent context, run the strongest available review and say so in the report — never present self-review as independent, never skip silently. A finding shape a review reports twice becomes a permanent automated guard — never caught by review a third time.
...
- Commands, runners, isolation, conventions: `docs/tests.md` (entry point; deep material under `docs/testing/`).
