...
A regression test reproduces the reported symptom through the boundary where it occurred and is seen to fail before the fix (or with the fix reverted). The failure must come from the defect, not from an import, fixture, or syntax error. If the only reachable boundary is too shallow to reproduce the real pattern, say so; that gap is a finding, not a reason for a test that passes by construction. If verification exposes an unrelated coverage gap, record it without turning the change into a test-wide campaign.
