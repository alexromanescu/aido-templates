...
7. **Clean up before reporting completion.** After merging, remove your worktree with `git worktree remove` and delete the merged scratch branch. Inventory everything the session created (processes, worktrees, branches, temp and scratch locations, fixtures left by tests) and verify each is gone; file a leak you did not cause instead of ignoring it. Never force-remove uncommitted or unmerged work or touch another session's resources. If your worktree must remain for unfinished work or review, record its path, branch, reason, and next cleanup action in the project's handoff record.
