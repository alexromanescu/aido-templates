...
**The loop runs autonomously between cycles** — after one cycle's review → fix → commit → reflect, start the next without asking. The only pauses are the 5-cycle checkpoint (ask) and natural termination at two consecutive zero-finding cycles (announce and exit).
...
The loop ends or pauses only on: (1) two consecutive zero-finding cycles → terminate and announce; (2) the 5-cycle checkpoint → pause, summarize tally and cost, ask; (3) a hard infrastructure error → report and pause. Nothing else — not diminishing findings, a small commit, or token concerns — ends it early. Equally, don't manufacture findings to keep the loop alive: a clean review is a real signal.
