...
## Boot (mirrors tests/helpers/boot-server.ts)

Create one task-specific disposable root, install cleanup immediately, and use
the repository-local executables. Ask the kernel for a currently free loopback
port; the readiness gates below also reject the reservation race where another
process takes that port before aido binds it.
...
created mode `0600` under `$AIDO_VERIFY_ROOT/mcp-config` only for its loopback
...
All implicit state belongs to the disposable root: `HOME`, XDG config, temp
files, project metadata, templates, ops data, MCP configs, and every legacy
migration source. Do not remove those overrides or point the running server at
a live checkout merely to simplify a fixture; copy committed inputs into the
disposable root instead.

The dedicated fail-fast subshell stays in the foreground after readiness. Its
exit trap stops the exact immutable process ID and removes only the immutable,
marker-validated `/tmp/aido-verify.*` root. Drive the printed URL from another
terminal, then press Ctrl-C in the owner terminal; do not detach or reassign
its cleanup state.
